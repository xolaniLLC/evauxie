        ...
        if(val.typeUtilisateur === 'vendor') {
          ...
          this.companyService.getCompanies().then(
            (docRef) => {
              ...
              if(docRef.length === 0) {
                const compName = new Company([val.email]);

                compName.nom = this.route.snapshot.queryParams['companyName'] ? this.route.snapshot.queryParams['companyName'] : '';
                compName.phone = this.route.snapshot.queryParams['companyPhoneNumber'] ? this.route.snapshot.queryParams['companyPhoneNumber'] : '';
                compName.pays = this.route.snapshot.queryParams['companyCountry'] ? this.route.snapshot.queryParams['companyCountry'] : '';

                this.listCompany.push(compName);
          ...
          this.router.navigateByUrl('../planning-tools');
        }
      }
    );
  }
